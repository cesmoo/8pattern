...
# Create routers for different permission levels
...
# ==========================================
# 🛡️ 3. PERMISSION SYSTEM (MIDDLEWARE)
# ==========================================
class AuthMiddleware(BaseMiddleware):
    ...
    async def __call__(self, handler, event, data):
        ...
        is_auth, level = await check_permission(user_id)
        ...
        if not is_auth:
            if isinstance(event, types.Message):
                ...
                await event.answer("🔒 Access Denied! Owner/Sudo only.", show_alert=True)
            return
...
async def get_user_balance(user_id: int) -> dict:
    ...
    if not user:
        user = {
            ...
            "lose_streak": 0, "best_streak": 0, "created_at": datetime.now()
        }
...
async def update_balance(user_id: int, amount: float, operation: str = "add") -> dict:
    ...
    await users_collection.update_one({"user_id": user_id}, {"$set": {"balance": new_balance}})
...
async def place_bet(user_id: int, issue_number: str, bet_amount: float, predicted_size: str, ai_mode: str) -> dict:
    ...
    await users_collection.update_one({"user_id": user_id}, {"$inc": {"total_bets": 1, "total_wagered": bet_amount}})
    return {"success": True, "message": f"{Emoji.CHECK} လောင်းကြေးထည့်ပြီးပါပြီ!", "balance": user['balance'] - bet_amount}
    ...
    for bet in pending_bets:
        ...
        if is_win:
            ...
            if user["win_streak"] > user["best_streak"]:
                await users_collection.update_one({"user_id": user_id}, {"$set": {"best_streak": user["win_streak"]}})
    ...
    try:
        ...
        if is_win:
            message = (
                ...
                f"━━━━━━━━━━━━━━━━━━\n"
                f"{Emoji.GAME_ICON} <b>{game_type}</b> : <code>{bet['issue_number']}</code>\n"
            ...
            message = (
                ...
                f"{Emoji.CHART_ICON} <b>Result:</b> {actual_number} {Emoji.BULLSEYE if actual_size == 'BIG' else Emoji.GREEN_CIRCLE} {actual_size} {color}\n"
                f"{Emoji.MONEY_ICON} <b>Balance:</b> {user['balance']:,.2f} Ks\n"
                f"{Emoji.LOSS_ICON} <b>Profit:</b> {user['profit']:,.2f} Ks"
            )
...
def detect_active_pattern(history_list):
    if len(history_list) < 4:
        return None, None
    patterns_to_check = [
        ("BBSS", ["BIG", "BIG", "SMALL", "SMALL"]), ("BBS", ["BIG", "BIG", "SMALL"]),
        ("BSS", ["BIG", "SMALL", "SMALL"]), ("BSBS", ["BIG", "SMALL", "BIG", "SMALL"]),
        ("SBSB", ["SMALL", "BIG", "SMALL", "BIG"]), ("BSB", ["BIG", "SMALL", "BIG"]),
        ("SBS", ["SMALL", "BIG", "SMALL"]), ("BBB", ["BIG", "BIG", "BIG"]),
    ...
    for pattern_name, pattern_seq in patterns_to_check:
        ...
        match_count = sum(1 for i in range(len(recent) - pattern_len + 1) if recent[i:i+pattern_len] == pattern_seq)
        if match_count >= 2:
            pattern_next_map = {"BBSS": "BIG", "BBS": "BIG", "BSS": "BIG", "BSBS": "BIG", "SBSB": "SMALL", "BSB": "BIG", "SBS": "SMALL", "BBB": "BIG", "SSS": "SMALL"}
...
def martingale_predict(history_docs):
    ...
    big_count = recent_10.count("BIG"); small_count = recent_10.count("SMALL")
...
def anti_martingale_predict(history_docs):
    ...
    for result in reversed(recent_5):
        if result == "BIG": big_streak += 1; small_streak = 0
...
def trend_following_predict(history_docs):
    ...
    docs = list(reversed(history_docs))
    all_history = [d.get('size', 'BIG') for d in docs]
    recent_8 = all_history[-8:]; recent_4 = all_history[-4:]
    big_8 = recent_8.count("BIG") / 8; big_4 = recent_4.count("BIG") / 4
    ...
    if trend_direction > 0.1:
        ...
        return latest, f"{latest} ({'အကြီး' if latest == 'BIG' else 'အသေး'}) {emoji_display}", 60.0, f"{Emoji.TREND} Sideways {Emoji.LEFT_RIGHT}"
...
def fibonacci_predict(history_docs):
    ...
    if results:
        ...
        return final, f"{final} ({'အကြီး' if final == 'BIG' else 'အသေး'}) {emoji_display}", 68.0, f"{Emoji.FIBONACCI} Fib levels: {len(results)}"
...
def momentum_predict(history_docs):
    ...
    for i, result in enumerate(all_history[-5:]):
        if result == "BIG": momentum_score += weights[i]
        else: momentum_score -= weights[i]
    if momentum_score > 3:
...
def wave_analysis_predict(history_docs):
    ...
    for result in all_history[1:]:
        ...
        else: waves.append((current, count)); current = result; count = 1
...
def chaos_theory_predict(history_docs):
    ...
    def calc_entropy(segment):
        total = len(segment)
        big_p = segment.count("BIG") / total; small_p = segment.count("SMALL") / total
        entropy = 0
        for p in [big_p, small_p]:
            if p > 0: entropy -= p * np.log2(p)
        return entropy
    entropy_3 = calc_entropy(all_history[-3:]); entropy_5 = calc_entropy(all_history[-5:]); entropy_10 = calc_entropy(all_history[-10:])
...
def generate_winrate_chart(predictions, ai_mode="pattern", user_data=None):
    ...
    for i, p in enumerate(latest_preds): 
        ...
        if 'WIN' in p.get('win_lose', ''):
            wins += 1; bar_colors.append('#00e5ff'); dots_list.append(('W', '#1de9b6'))
        else:
            losses += 1; bar_colors.append('#ff4444'); dots_list.append(('L', '#ef5350'))
    ...
    if user_data:
        balance = user_data.get('balance', 0); profit = user_data.get('profit', 0)
    ...
    ax_wm.plot([0.1, 0.9], [0.30, 0.30], color='#ffd700', lw=3); ax_wm.plot([0.1, 0.9], [0.70, 0.70], color='#ffd700', lw=3)
...
async def check_game_and_predict(session: aiohttp.ClientSession):
    ...
    if not PREDICTION_ACTIVE: return False
    ...
    json_data = {
        ...
        'random': '9ef85244056948ba8dcae7aee7758bf4', 'signature': '2EDB8C2B5264F62EC53116916A9EC05C', 'timestamp': int(time.time()),
    }
        ...
        if records:
            ...
            if is_new_issue:
                ...
                await history_collection.update_one(
                    {"issue_number": latest_issue}, {"$setOnInsert": {"number": latest_number, "size": latest_size}}, upsert=True
                )
                settled = await settle_bets(latest_issue, latest_size, latest_number, "WINGO_30S")
                if settled > 0: print(f"💰 Settled {settled} bets for {latest_issue} - {latest_number} ({latest_size})")
                ...
                await predictions_collection.update_one(
                    {"issue_number": next_issue}, {"$set": {"predicted_size": predicted_size, "ai_mode": CURRENT_AI_MODE}}, upsert=True
                )
                for user_id in list(ACTIVE_USERS):
                    try:
                        ...
                        for bet in recent_bets:
                            if bet["result"] == "LOSE": lose_streak += 1
                            else: break
                        ...
                        if user["balance"] >= bet_amount:
                            ...
                            if bet_result["success"]:
                                ...
                                try: await bot.send_message(chat_id=user_id, text=order_msg)
                                except: pass
                    except Exception as e: print(f"Auto-bet error for {user_id}: {e}")
...
async def update_channel_post(next_issue, predicted_display, final_prob, reason):
    ...
    try:
        pred_cursor = predictions_collection.find({
            "issue_number": {"$gte": SESSION_START_ISSUE}, "win_lose": {"$ne": None}
        }).sort("issue_number", -1)
        ...
        for p in session_preds[:10]: 
            ...
            act_size = p.get('actual_size', 'BIG'); act_num = p.get('actual_number', 0)
        ...
        if MAIN_MESSAGE_ID:
            ...
            msg = await bot.send_photo(chat_id=CHANNEL_ID, photo=photo, caption=tg_caption)
            MAIN_MESSAGE_ID = msg.message_id
    except Exception as e: print(f"Channel update error: {e}")

# ==========================================
# ⏱️ 9. SCHEDULER
# ==========================================
async def auto_broadcaster():
    await init_db(); await load_sudo_users()
            ...
            if PREDICTION_ACTIVE and 5 <= sec_passed <= 28:
                try:
                    ...
                    if is_processed:
                        ...
                        await asyncio.sleep(sleep_time); continue 
                except Exception as e: pass
            await asyncio.sleep(0.5) 

# ==========================================
# 🤖 10. AUTH COMMANDS (Owner + Sudo)
# ==========================================
...
async def send_welcome(message: types.Message):
    ...
    await message.reply(
        ...
        f"{Emoji.GAME_ICON} <b>Commands:</b>\n"
...
@auth_router.message(lambda message: message.text and message.text.lower() == '.active')
async def activate_user(message: types.Message):
    ...
    if user_id in ACTIVE_USERS:
        ...
        return
    ACTIVE_USERS.add(user_id); PREDICTION_ACTIVE = True
    await active_sessions_collection.update_one(
        {"user_id": user_id}, {"$set": {"active": True, "activated_at": datetime.now(), "ai_mode": CURRENT_AI_MODE}}, upsert=True
    )
    user = await get_user_balance(user_id)
    ai_emoji = AI_MODES.get(CURRENT_AI_MODE, {}).get("name", "")
    await message.reply(
...
@auth_router.message(lambda message: message.text and message.text.lower() == '.stop')
async def deactivate_user(message: types.Message):
    ...
    await active_sessions_collection.update_one(
        {"user_id": user_id}, {"$set": {"active": False, "stopped_at": datetime.now()}}, upsert=True
    )
    if not ACTIVE_USERS: PREDICTION_ACTIVE = False
    user = await get_user_balance(user_id)
...
async def change_mode(message: types.Message):
    ...
    builder.row(InlineKeyboardButton(text=f"{Emoji.PATTERN} Pattern", callback_data="mode_pattern"), InlineKeyboardButton(text=f"{Emoji.MARTINGALE} Martingale", callback_data="mode_martingale"))
    builder.row(InlineKeyboardButton(text=f"{Emoji.ANTIMARTINGALE} Anti-Martingale", callback_data="mode_anti_martingale"), InlineKeyboardButton(text=f"{Emoji.TREND} Trend Follow", callback_data="mode_trend_following"))
    builder.row(InlineKeyboardButton(text=f"{Emoji.FIBONACCI} Fibonacci", callback_data="mode_fibonacci"), InlineKeyboardButton(text=f"{Emoji.GOLDEN} Golden Ratio", callback_data="mode_golden_ratio"))
    builder.row(InlineKeyboardButton(text=f"{Emoji.MOMENTUM} Momentum", callback_data="mode_momentum"), InlineKeyboardButton(text=f"{Emoji.MONTECARLO} Monte Carlo", callback_data="mode_monte_carlo"))
    builder.row(InlineKeyboardButton(text=f"{Emoji.NEURAL} Neural Pattern", callback_data="mode_neural_pattern"), InlineKeyboardButton(text=f"{Emoji.REVERSAL} Quick Reversal", callback_data="mode_quick_reversal"))
    builder.row(InlineKeyboardButton(text=f"{Emoji.WAVE} Wave Analysis", callback_data="mode_wave_analysis"), InlineKeyboardButton(text=f"{Emoji.CHAOS} Chaos Theory", callback_data="mode_chaos_theory"))
...
@auth_router.callback_query(lambda c: c.data.startswith("mode_"))
...
@auth_router.message(Command("compare"))
async def compare_ai_modes(message: types.Message):
    ...
    for i, (mode_key, data) in enumerate(sorted_results, 1):
        ...
        compare_text += f"{medal} {data['name']}{current}\n   {Emoji.BAR_CHART} {data['accuracy']:.1f}% ({data['correct']}/{data['total']})\n"
...
async def show_leaderboard(message: types.Message):
    ...
    for i, user in enumerate(leaderboard, 1):
        ...
        top_text += f"{medal} <code>{user['user_id']}</code>\n   {Emoji.MONEY} {user['balance']:,.0f} Ks | {profit_e} {user.get('profit', 0):,.0f} | {Emoji.BAR_CHART} {wr:.1f}%\n"
    await message.reply(top_text)

@auth_router.message(lambda message: message.text and message.text.startswith('.bet'))
async def place_bet_command(message: types.Message):
    global LAST_PROCESSED_ISSUE, CURRENT_AI_MODE
    if not BETTING_ENABLED:
        await message.reply(f"{Emoji.OFFLINE} Virtual Betting ပိတ်ထားပါသည်။"); return
    try:
        parts = message.text.split()
        if len(parts) < 2:
            await message.reply(f"{Emoji.INFO} <code>.bet 100</code> or <code>.bet 100-300-900</code>"); return
        bet_params = parts[1]
        if '-' in bet_params:
            ...
            bet_amount = float(bet_params); streak_info = ""
    ...
    except ValueError: await message.reply(f"{Emoji.CROSS} ဂဏန်းများသာ ထည့်ပါ။")

@auth_router.message(lambda message: message.text and message.text.startswith('.bal'))
async def check_balance(message: types.Message):
    ...
    await message.reply(
        ...
        f"{Emoji.FIRE} Best Streak: {user.get('best_streak', 0)}\n"
        f"━━━━━━━━━━━━━━━━━━\n{Emoji.INFO} <code>.addbal 50000</code> | <code>.withdraw 50000</code>"
    )

@auth_router.message(lambda message: message.text and message.text.startswith('.addbal'))
async def add_balance(message: types.Message):
    try:
        parts = message.text.split()
        if len(parts) < 2:
            await message.reply(f"{Emoji.INFO} <code>.addbal 50000</code>"); return
        amount = float(parts[1])
        if amount <= 0 or amount > 1000000:
            await message.reply(f"{Emoji.CROSS} 1 - 1,000,000 Ks အတွင်းသာထည့်ပါ။"); return
        user = await update_balance(message.from_user.id, amount, "add")
        await message.reply(f"{Emoji.CHECK} <b>ငွေထည့်ပြီးပါပြီ!</b>\n\n{Emoji.MONEY_BAG} +{amount:,.0f} Ks\n{Emoji.MONEY_ICON} Balance: {user['balance']:,.0f} Ks")
    except ValueError: await message.reply(f"{Emoji.CROSS} ဂဏန်းများသာ ထည့်ပါ။")

@auth_router.message(lambda message: message.text and message.text.startswith('.withdraw'))
async def withdraw_balance(message: types.Message):
    try:
        parts = message.text.split()
        if len(parts) < 2:
            await message.reply(f"{Emoji.INFO} <code>.withdraw 50000</code> or <code>.withdraw all</code>"); return
        user = await get_user_balance(message.from_user.id)
        if parts[1].lower() == "all":
            amount = user['balance']
            if amount <= 0: await message.reply(f"{Emoji.CROSS} နှုတ်ယူရန် လက်ကျန်ငွေမရှိပါ!"); return
        else:
            amount = float(parts[1])
            if amount <= 0: await message.reply(f"{Emoji.CROSS} 0 ထက်ကြီးရပါမည်။"); return
        if amount > user['balance']: await message.reply(f"{Emoji.CROSS} လက်ကျန်ငွေ မလုံလောက်ပါ!"); return
        ...
        await message.reply(
            ...
            f"{Emoji.MONEY_BAG} နှုတ်ယူငွေ: <b>-{amount:,.0f} Ks</b>\n{Emoji.MONEY_ICON} လက်ကျန်ငွေ: <b>{updated_user['balance']:,.0f} Ks</b>"
        )
    except ValueError: await message.reply(f"{Emoji.CROSS} ဂဏန်းများသာ ထည့်ပါ။")
...
async def show_my_bets(message: types.Message):
    ...
    if not bets: await message.reply(f"{Emoji.INFO} မှတ်တမ်းမရှိသေးပါ။"); return
    text = f"{Emoji.GAME} <b>လောင်းကြေးမှတ်တမ်း</b>\n━━━━━━━━━━━━━━━━━━\n"
    for bet in bets:
        if bet["result"] is None: status = "⏳ Pending"
        elif bet["result"] == "WIN": status = f"{Emoji.CHECK} +{bet['profit']:,.0f} Ks"
        else: status = f"{Emoji.CROSS} -{bet['bet_amount']:,.0f} Ks"
        text += f"{bet['issue_number']}: {bet['bet_amount']:,.0f}K on {bet['predicted_size']} {Emoji.UP} {status}\n"
    await message.reply(text)

# ==========================================
# 🛡️ 11. OWNER ONLY COMMANDS
# ==========================================
@owner_router.message(Command("addsudo"))
async def add_sudo_command(message: types.Message):
    try:
        parts = message.text.split()
        if len(parts) < 2: await message.reply(f"{Emoji.INFO} <code>/addsudo [user_id]</code>"); return
        target_id = int(parts[1])
        if str(target_id) == str(OWNER_ID): await message.reply(f"{Emoji.CROSS} Owner ကို Sudo ထည့်၍မရပါ!"); return
        if target_id in SUDO_USERS: await message.reply(f"{Emoji.CROSS} ဤ User သည် Sudo ဖြစ်ပြီးသားပါ!"); return
        success = await add_sudo_user(target_id, message.from_user.id)
        if success:
            await message.reply(f"{Emoji.CHECK} <b>Sudo User ထည့်ပြီးပါပြီ!</b>\n\n{Emoji.SHIELD} User: <code>{target_id}</code>\n{Emoji.INFO} ယခု Bot ကိုအသုံးပြုခွင့်ရပါပြီ။")
            try: await bot.send_message(chat_id=target_id, text=f"{Emoji.SPARKLES} <b>Congratulations!</b>\n\n{Emoji.SHIELD} သင့်အား Sudo User အဖြစ်ထည့်သွင်းလိုက်ပါပြီ!\n{Emoji.KEY} ယခု Bot ကိုအသုံးပြုခွင့်ရပါပြီ။\n\n{Emoji.INFO} /start ဖြင့်စတင်ပါ။")
            except: pass
        else: await message.reply(f"{Emoji.CROSS} ထည့်၍မရပါ!")
    except ValueError: await message.reply(f"{Emoji.CROSS} User ID ဂဏန်းသာထည့်ပါ။")

@owner_router.message(Command("delsudo"))
async def remove_sudo_command(message: types.Message):
    try:
        parts = message.text.split()
        if len(parts) < 2: await message.reply(f"{Emoji.INFO} <code>/delsudo [user_id]</code>"); return
        target_id = int(parts[1])
        if target_id not in SUDO_USERS: await message.reply(f"{Emoji.CROSS} ဤ User သည် Sudo မဟုတ်ပါ!"); return
        await remove_sudo_user(target_id)
        await message.reply(f"{Emoji.CHECK} <b>Sudo User ဖယ်ရှားပြီးပါပြီ!</b>\n\n{Emoji.BANNED} User: <code>{target_id}</code>\n{Emoji.INFO} ယခု Bot ကိုအသုံးပြုခွင့်မရတော့ပါ။")
        try: await bot.send_message(chat_id=target_id, text=f"{Emoji.WARNING} <b>Notice!</b>\n\n{Emoji.BANNED} သင့်အား Sudo User မှဖယ်ရှားလိုက်ပါပြီ။\n{Emoji.INFO} ယခု Bot ကိုအသုံးပြုခွင့်မရတော့ပါ။")
        except: pass
    except ValueError: await message.reply(f"{Emoji.CROSS} User ID ဂဏန်းသာထည့်ပါ။")
...
async def list_sudo_users(message: types.Message):
    global SUDO_USERS
    if not SUDO_USERS: await message.reply(f"{Emoji.INFO} Sudo Users မရှိသေးပါ။"); return
    sudo_list = f"{Emoji.SHIELD} <b>SUDO USERS LIST</b>\n━━━━━━━━━━━━━━━━━━\n"
    for i, user_id in enumerate(SUDO_USERS, 1): sudo_list += f"{i}. <code>{user_id}</code>\n"
    sudo_list += f"━━━━━━━━━━━━━━━━━━\n📊 Total: <b>{len(SUDO_USERS)}</b> Sudo Users\n\n{Emoji.INFO} <code>/addsudo [id]</code> | <code>/delsudo [id]</code>"
...
async def set_balance(message: types.Message):
    try:
        ...
        if len(parts) == 2:
            amount = float(parts[1])
            if amount < 0: await message.reply(f"{Emoji.CROSS} 0 သို့မဟုတ် အပေါင်းကိန်းဖြစ်ရပါမည်။"); return
        ...
        else: await message.reply(f"{Emoji.INFO} <code>/setbal 50000</code> or <code>/setbal [id] 50000</code>")
    except ValueError: await message.reply(f"{Emoji.CROSS} ဂဏန်းများသာ ထည့်ပါ။")
...
async def give_money(message: types.Message):
    try:
        parts = message.text.split()
        if len(parts) < 3: await message.reply(f"{Emoji.INFO} <code>/give [user_id] [amount]</code>"); return
        target_id = int(parts[1]); amount = float(parts[2])
        if amount <= 0: await message.reply(f"{Emoji.CROSS} 0 ထက်ကြီးရပါမည်။"); return
        receiver = await update_balance(target_id, amount, "add")
        await message.reply(f"{Emoji.CHECK} <b>ငွေထည့်ပေးပြီးပါပြီ!</b>\n\n👤 User: <code>{target_id}</code>\n{Emoji.MONEY_BAG} +{amount:,.0f} Ks\n{Emoji.MONEY_ICON} Balance: {receiver['balance']:,.0f} Ks")
        try: await bot.send_message(chat_id=target_id, text=f"🎁 <b>ငွေထည့်ပေးခြင်းခံရပါသည်!</b>\n━━━━━━━━━━━━━━━━━━\n{Emoji.MONEY_BAG} +{amount:,.0f} Ks\n{Emoji.MONEY_ICON} Balance: {receiver['balance']:,.0f} Ks\n━━━━━━━━━━━━━━━━━━\n{Emoji.CROWN} Owner မှ ထည့်ပေးခြင်းဖြစ်ပါသည်။")
        except: pass
    except ValueError: await message.reply(f"{Emoji.CROSS} <code>/give [user_id] [amount]</code>")

@owner_router.message(Command("reset"))
async def reset_user_stats(message: types.Message):
    try:
        parts = message.text.split()
        if len(parts) == 1: target_id = message.from_user.id
        elif len(parts) == 2: target_id = int(parts[1])
        else: await message.reply(f"{Emoji.INFO} <code>/reset</code> or <code>/reset [user_id]</code>"); return
        await users_collection.update_one({"user_id": target_id}, {"$set": {"total_bets": 0, "total_wins": 0, "total_losses": 0, "total_wagered": 0.0, "total_won": 0.0, "profit": 0.0, "win_streak": 0, "lose_streak": 0, "best_streak": 0}})
        await bets_collection.delete_many({"user_id": target_id})
        user = await get_user_balance(target_id)
        await message.reply(f"{Emoji.CHECK} <b>Statistics ပြန်လည်သတ်မှတ်ပြီးပါပြီ!</b>\n\n👤 User: <code>{target_id}</code>\n{Emoji.MONEY_ICON} Balance: <b>{user['balance']:,.0f} Ks</b>\n{Emoji.INFO} Stats & Bet History ဖျက်လိုက်ပါပြီ။")
    except ValueError: await message.reply(f"{Emoji.CROSS} ဂဏန်းများသာ ထည့်ပါ။")
...
async def broadcast_message(message: types.Message):
    try:
        ...
        if len(parts) < 2: await message.reply(f"{Emoji.INFO} <code>/broadcast [message]</code>"); return
        ...
        for uid in user_ids:
            try:
                await bot.send_message(chat_id=uid, text=f"{Emoji.SPARKLES} <b>BROADCAST</b> {Emoji.SPARKLES}\n━━━━━━━━━━━━━━━━━━\n{broadcast_text}\n━━━━━━━━━━━━━━━━━━\n{Emoji.CROWN} Owner Message")
                success += 1
            except: failed += 1
            await asyncio.sleep(0.05)
        await message.reply(f"{Emoji.CHECK} <b>Broadcast Sent!</b>\n✅ Success: {success}\n❌ Failed: {failed}")
    except Exception as e: await message.reply(f"{Emoji.CROSS} Error: {str(e)}")

@owner_router.message(Command("togglebet"))
async def toggle_betting(message: types.Message):
    global BETTING_ENABLED
    BETTING_ENABLED = not BETTING_ENABLED
    await settings_collection.update_one({"key": "betting_enabled"}, {"$set": {"value": BETTING_ENABLED}}, upsert=True)
...
async def init_db():
    try:
        ...
        global CURRENT_AI_MODE
        if setting: CURRENT_AI_MODE = setting.get("value", "pattern")
        bet_setting = await settings_collection.find_one({"key": "betting_enabled"})
        global BETTING_ENABLED
        if bet_setting: BETTING_ENABLED = bet_setting.get("value", True)
        global ACTIVE_USERS
        ...
        if ACTIVE_USERS: PREDICTION_ACTIVE = True
...
async def main():
    print(f"\n{Emoji.SPARKLES} WIN GO AI Bot v3.0 {Emoji.SPARKLES}")
    print(f"{Emoji.CROWN} Owner: {OWNER_ID}")
    print(f"{Emoji.LOCK} Only Owner & Sudo Users can access!")
    print(f"{Emoji.ROBOT} 12 AI Modes | {Emoji.MONEY_ICON} Virtual Balance | {Emoji.SHIELD} Middleware System\n")
